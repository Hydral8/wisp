...
def _apply_saved_credentials(node: DAGNode, args: dict[str, Any]) -> dict[str, Any]:
    ...
    # Fill top-level auth fields only when absent/blank.
    for key, value in cred_values.items():
        existing = merged.get(key)
        if existing is None or (isinstance(existing, str) and not existing.strip()):
            merged[key] = value

    # Also merge into nested `credentials` object when present.
...
async def run_workflow(wf: Workflow):
    ...
    for li, level in enumerate(levels):
        ...
        for node_events in await asyncio.gather(*[run_regular_node(n, li) for n in regular_nodes]):
